 ...
 * 个人实体维度表写入HBase
 */
class IndividualSink extends Sink[FinancialEntity] {
  override def doSink(input: DataStream[FinancialEntity]): Unit =
    input.addSink(new IndividualSinkFunction)
}

class IndividualSinkFunction extends RichSinkFunction[FinancialEntity] {
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    table = conn.getTable(TableName.valueOf("dim:individuals"))
  }
  ...
  override def invoke(entity: FinancialEntity, context: SinkFunction.Context): Unit =
    try {
      ...
      case e: Exception => LoggerUtil.error(logger, e,
        s"Failed to write individual entity: $entity")
    }
